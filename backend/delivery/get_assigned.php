...
try {
    ...
    // Get orders assigned to this livreur that are not yet delivered
    $stmt = $pdo->prepare("
        SELECT
            c.ID_Commande,
            c.Date_Commande,
        ...
        WHERE l.ID_Livreur = ? 
        AND l.Statut_Livraison NOT IN ('Livrée', 'Annulée')
        ORDER BY l.Date_Debut_Livraison ASC
    ");
    ...
    echo json_encode(['error' => 'Failed to fetch assigned orders: ' . $e->getMessage()]);
}
?>
